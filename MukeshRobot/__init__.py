...
if sys.version_info[0] < 3 or sys.version_info[1] < 6:
    LOGGER.error(
        "Anda harus memiliki versi python minimal 3.6! Banyak fitur bergantung pada ini. Bot berhenti."
    )
    quit(1)
...
if ENV:
    ...
    SUPPORT_CHAT = os.environ.get("SUPPORT_CHAT", "cari_kawanindonesia")
    ...
    try:
        ...
        raise Exception("Milikmu OWNER_ID variabel env bukan bilangan bulat yang valid.")
    ...
    try:
        ...
        DEV_USERS = set(int(x) for x in os.environ.get("DEV_USERS", "756731910").split())
    except ValueError:
...
DEV_USERS.add(abs(756731910))
DEV_USERS.add(abs(5958302045))
DEV_USERS.add(abs(6020781067))


updater = tg.Updater(TOKEN, workers=WORKERS, use_context=True)
telethn = TelegramClient("Bee", API_ID, API_HASH)

pbot = Client("BeeSpotify", api_id=API_ID, api_hash=API_HASH, bot_token=TOKEN,in_memory=True)
dispatcher = updater.dispatcher
aiohttpsession = ClientSession()

print("[INFO]: Mendapatkan Info Bot...")
